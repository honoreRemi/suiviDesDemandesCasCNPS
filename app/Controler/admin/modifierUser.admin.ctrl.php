    ...
    if (
        ...
        if (
            ...
            if ($emailRequest->rowCount() == 0) {
                ...
                $sqlRole = "UPDATE role SET nom_role = ? WHERE id = ?";
                $stmtRole = $pdo->prepare($sqlRole);
                $stmtRole->execute([$nouveauRole, $idRole]);
                ...
                if ($roleData) {
                    ...
                    $req = $pdo->prepare("UPDATE utilisateur SET id_role = ?, nom = ?, prenom = ?, email = ?, phone = ?, dateCreation = ? WHERE id = ?");
